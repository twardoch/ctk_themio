...
# This module is the single source the view layer reaches through for path
# constants (e.g. ``mod.APP_IMAGES``). Re-export the full set from paths so the
# GUI keeps its facade; ``__all__`` marks them so the linter treats them as
# public re-exports rather than unused imports.
...
log.log_debug(log_text=f"APP_HOME={APP_HOME}", class_name="ctk_theme_builder.py")
...
DISCONNECT_JSON = (
    '{"command_type": "program", "command": "' + DISCONNECT_MESSAGE + '", "parameters": [""]}'
)
...
def complete_qa_stop():
    time.sleep(0.5)
    log.log_debug(
        log_text="Slept, now calling remove_qa_status_files() and closing QA app",
        class_name="ctk_theme_builder.py",
...
def os_attribute(attribute: str = "os_name"):
    ...
    if attribute not in valid_attributes:
        raise AttributeError(
            f"Invalid attribute received: {attribute}; expected one of {', '.join[valid_attributes]}"
        )
...
def remove_qa_status_files():
    if QA_STOP_FILE.exists():
        try:
            log.log_debug(
                log_text="Removing QA_STOP_FILE",
                class_name="ctk_theme_builder.py",
    ...
    if QA_STARTED_FILE.exists():
        try:
            log.log_debug(
                log_text="Removing QA_STARTED_FILE",
                class_name="ctk_theme_builder.py",
                method_name="remove_qa_status_files",
            )
...
def patch_theme(theme_json: dict):
    ...
    :return:"""
    if (
        "CTkCheckbox" in theme_json
        or "CTkRadiobutton" in theme_json
...
def cascade_dict(palette_id: int) -> list:
    ...
    cur = db_conn.cursor()
    cur.execute(
        "select widget_type, widget_property "
...
def cascade_enabled(palette_id: int) -> bool:
    ...
    cur = db_conn.cursor()
    cur.execute(
        "select count(*) from colour_cascade_properties where entry_id = :entry_id",
        {"entry_id": palette_id},
    )
    ...
    for widget_type in SUPPORTED_WIDGETS:
        ...
        for widget_property in COLOUR_PROPERTIES:
            if keys_exist(theme_dict, j_widget_type, widget_property):
                ...
                _colour_dictionary[f"{disp_widget}: {widget_property}"] = theme_dict[j_widget_type][
                    widget_property
                ]
...
def merge_themes(
    ...
    for widget_type, dict_ in new_theme_dict.items():
        for property_, value_ in dict_.items():
            if "_color" in property_ and value_ != "transparent":
                new_theme_dict[widget_type][property_][mapped_primary_idx] = primary_theme_dict[
                    widget_type
                ][property_][primary_mode_idx]
                new_theme_dict[widget_type][property_][mapped_secondary_idx] = secondary_theme_dict[
                    widget_type
                ][property_][secondary_mode_idx]
...
def user_themes_list():
    ...
    user_themes_dir = pref.preference_setting(
        db_file_path=DB_FILE_PATH, scope="user_preference", preference_name="theme_json_dir"
    )
...
def listener_port():
    ...
    Preview Panel."""
    _listener_port = pref.preference_setting(
        db_file_path=DB_FILE_PATH, scope="user_preference", preference_name="listener_port"
    )
    log.log_debug(
        log_text=f"Listener port returned as {_listener_port}",
        class_name="ctk_theme_builder.py",
...
def method_listener_address():
    ...
    log.log_debug(
        log_text=f"Listener address returned as {_method_listener_address}",
        class_name="ctk_theme_builder.py",
        method_name="listener_port",
    )
...
def send_command_json(command_type: str, command: str, parameters: list = None):
    ...
    log.log_debug(
        log_text=f"Parameters: command_type={command_type}, command={command}, parameters={parameters}",
        class_name="ctk_theme_builder.py",
    ...
    message_json_str = (
        '{ "command_type": "%command_type%", "command": "%command%", "parameters": [%parameters%] }'
    )
    ...
    log.log_debug(
        log_text=f"Sending message; message={message_json_str}",
        class_name="ctk_theme_builder.py",
        method_name="send_command_json",
    )
...
def colour_palette_entries(db_file_path: Path):
    ...
    cur.execute("select entry_id, row, col, label from colour_palette_entries order by entry_id;")
...
def send_message(message):
    ...
    while not connected:
        try:
            if connect_tries > 10:
                print(
                    f"Communication error sending message to preview panel, via {METHOD_LISTENER_ADDRESS}!"
                )
                exit(1)
            connect_tries += 1
...
class PropertyVector:
    ...
    def __repr__(self):
        return (
            f"{self.__class__.__name__} of {self._display_property()} (old_value = {self.old_value}, "
    ...
    def this_display_property(self):
        if self.command not in ("colour", "geometry"):
            ValueError(
                f"{__class__}: vector with command defined as, {self.command}, "
                f"not supported by method, display_property."
            )
        _widget_type = self.component_type
        _widget_property = self.component_property

        return PropertyVector.display_property(
            widget_type=_widget_type, widget_property=_widget_property
        )
...
class CommandStack:
    ...
    def undo_command(self) -> tuple:
        ...
        log.log_debug(
            log_text="Undo command vector", class_name="CommandStack", method_name="undo_command"
        )
        ...
        if _command_type == "palette_colour":
            ...
            return (
                f"{cap_command_type} change to {_component_type} / {_component_property}, reverted from "
        ...
        self.do_command(
            ...
            component_property=_component_property,
            property_value=_old_property_value,
        )
        self.redo_stack.append(_vector)
    ...
    def redo_command(self) -> tuple:
        ...
        log.log_debug(
            log_text="Redo command vector", class_name="CommandStack", method_name="redo_command"
        )
        if _command_type == "palette_colour":
            # This is a theme colour palette change, so we don't execute a preview panel command.
            # Instead, just return the required reversal properties.
        ...
        self.do_command(
            ...
            component_type=_component_type,
            component_property=_component_property,
            property_value=_new_property_value,
        )
        self.undo_stack.append(_vector)

        if _command_type in ("colour", "geometry"):
            return (
                f"{cap_command_type} change to {_component_type} / {_component_property}, rolled forward from "
    ...
    def do_command(
        ...
        send_command_json(command_type=command_type, command=command, parameters=parameters)
...
if __name__ == "__main__":
    ...
    sorted_widget_properties_x = {
        "Button: border_color": ["#100000", "#100000"],
    ...
    counter = 0
    member_gen = widget_member(
        widget_entries=sorted_widget_properties_x, filter_list=properties_filter_list_x
    )
    ...
    sample_setting = pref.preference_setting(
        db_file_path=db_file, scope="window_geometry", preference_name="control_panel"
    )
    print(f"Sample geometry = {sample_setting}")
    sample_row = pref.preference_row(
        db_file_path=db_file, scope="window_geometry", preference_name="control_panel"
    )
