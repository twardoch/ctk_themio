...
class HarmonicsDialog(ctk.CTkToplevel):
    def __init__(self, theme_name, theme_json_data: dict, *args, **kwargs):
        ...
        self.frm_shades_palette.grid(
            column=2, row=0, padx=(0, 10), pady=10, sticky="nsew", rowspan=2
        )
        ...
        self.harmony_status_bar = cbtk.CBtkStatusBar(
            master=self, status_text_life=30, use_grid=True
        )

        self.new_theme_json_dir = None

        lbl_keystone_header = ctk.CTkLabel(
            master=frm_controls, text="Keystone Colour", font=HEADING4, justify=tk.CENTER
        )

        lbl_keystone_header.grid(row=0, column=0, sticky="ew", pady=5, padx=(25, 25))

        lbl_harmony_header = ctk.CTkLabel(
            master=self.frm_harmony_colours, text="Colour Harmony", font=HEADING4, justify=tk.CENTER
        )

        lbl_harmony_header.grid(row=0, column=0, sticky="ew", pady=5, padx=(25, 25))

        lbl_keystone_colour = ctk.CTkLabel(
            master=frm_controls, text="Paste / select color:", font=REGULAR_TEXT
        )
        lbl_keystone_colour.grid(row=1, column=0, sticky="w", pady=0, padx=(25, 0))

        bg_colour = self.theme_json_data.get("provenance", {}).get("keystone colour", None)
        self.btn_keystone_colour = ctk.CTkButton(
            master=frm_controls,
            border_width=3,
            fg_color=bg_colour,
            width=120,
        ...
        self.opm_harmony_method = ctk.CTkOptionMenu(
            master=frm_controls,
            values=["Analogous", "Complementary", "Split-complementary", "Triadic", "Tetradic"],
            command=self.switch_harmony_method,
        )
        self.opm_harmony_method.grid(row=3, column=0, padx=(10, 10), pady=(0, 10))
        harmony_method = self.theme_json_data.get("provenance", {}).get(
            "harmony method", "Analogous"
        )

        self.opm_harmony_method.set(harmony_method)

        cbtk.CBtkToolTip(self.btn_keystone_colour, text="Right click for options.")
        mnu_keystone = cbtk.CBtkMenu(self.btn_keystone_colour, tearoff=False)
        mnu_keystone.add_command(label="Copy", command=self.copy_harmony_input_colour)

        mnu_keystone.add_command(label="Paste", command=self.paste_harmony_keystone_colour)

        mnu_keystone.add_command(label="Colour Picker", command=self.harmony_input_colour_picker)
        ...
        self.btn_keystone_colour.bind(
            "<Button-3>", lambda event, menu=mnu_keystone: self.context_menu(event, menu)
        )
        ...
        btn_close = ctk.CTkButton(master=frm_buttons, text="Close", command=self.close_harmonics)

        btn_close.grid(row=0, column=0, padx=15, pady=5)

        btn_copy_to_palette = ctk.CTkButton(
            master=frm_buttons,
        ...
        if "provenance" in self.theme_json_data:
            btn_save_keystone = ctk.CTkButton(
                master=frm_buttons,
                text="Tag Keystone",
                state=button_state,
    ...
    def copy_harmony_input_colour(self, event=None):
        ...
        log.log_debug(
            log_text=f"Copy harmony input colour, {colour} to clipboard",
            class_name="HarmonicsDialog",
            method_name="copy_harmony_input_colour",
        )
        pyperclip.copy(colour)
        self.harmony_status_bar.set_status_text(status_text=f"Colour {colour} copied to clipboard.")
    ...
    def paste_harmony_keystone_colour(self):
        ...
        log.log_debug(
            log_text=f"Paste harmony input colour, {new_colour} from clipboard",
            class_name="HarmonicsDialog",
        ...
        self.harmony_status_bar.set_status_text(status_text=f"Colour {new_colour} assigned.")

    @log_call
    def set_harmony_keystone(self, colour_code: str, method: str):
        hover_colour = cbtk.contrast_colour(colour_code)
        if colour_code:
            self.btn_keystone_colour.configure(fg_color=colour_code, hover_color=hover_colour)
        ...
        if method:
            ...
            log.log_debug(
                log_text=f"Set harmony keystone colour: {colour_code}",
                class_name="HarmonicsDialog",
    ...
    def copy_keystone_colour(self, event=None, harmony_button_id=None, shade_copy=False):
        ...
        log.log_debug(
            log_text=f"Copy keystone colour: {colour}",
            class_name="HarmonicsDialog",
    ...
    def copy_harmony_colour(self, event=None, harmony_button_id=None, shade_copy=False):
        ...
        log.log_debug(
            log_text=f"Copy harmony colour: {colour}",
            class_name="HarmonicsDialog",
    ...
    def harmony_input_colour_picker(self):
        ...
        log.log_debug(
            log_text="Launch harmony colour picker",
            class_name="HarmonicsDialog",
        ...
        self.lift()
        self.transient()
        primary_colour = askcolor(
            master=self, initialcolor=self.keystone_colour, title="Harmony Source Colour Selection"
        )

        if primary_colour[1] is not None:
            primary_colour = primary_colour[1]
            self.btn_keystone_colour.configure(fg_color=primary_colour, hover_color=primary_colour)
            self.harmony_status_bar.set_status_text(
    ...
    def harmony_colour_picker(self, palette_button_id):
        # self.harmonics_label_count
        primary_colour = askcolor(master=self, title="Harmony Source Colour Selection")

        if primary_colour[1] is not None:
            primary_colour = primary_colour[1]
            self.rendered_harmony_buttons[0].configure(
                fg_color=primary_colour, hover_color=primary_colour
            )
            self.harmony_status_bar.set_status_text(
    ...
    def populate_harmony_colours(self):
        log.log_debug(
            log_text="Populate harmony colours",
            class_name="HarmonicsDialog",
        ...
        for btn_idx in range(harmony_entries):
            ...
            self.rendered_harmony_buttons[btn_idx].configure(
                fg_color=harmony_colour, hover_color=harmony_colour
            )
        self.render_keystone_shades_palette(
            keystone_colour=primary_colour, harmony_method=harmony_method
        )
    ...
    def save_harmonics_geometry(self):
        ...
        log.log_debug(
            log_text="Save harmonics display geometry",
            class_name="HarmonicsDialog",
    ...
    def close_harmonics(self, event=None):
        log.log_debug(
            log_text="Closing colour harmonics dialogue",
            class_name="HarmonicsDialog",
            method_name="close_harmonics",
        )
    ...
    def switch_harmony_method(self, event="event"):
        ...
        harmony_method = self.opm_harmony_method.get()
        log.log_debug(
            log_text="Update the newly generated colours to buttons, below the keystone colour button",
            class_name="HarmonicsDialog",
        ...
        for btn_idx in range(harmony_entries):
            btn_palette = ctk.CTkButton(
                master=self.frm_harmony_colours,
                border_width=3,
                width=120,
                height=90,
                corner_radius=15,
            ...
            cbtk.CBtkToolTip(btn_palette, text="Right click for copy option.")
            ...
            menus[btn_idx].add_command(
                label="Copy",
                command=lambda button_id=btn_idx: self.copy_harmony_colour(
                    harmony_button_id=button_id
                ),
            )

            btn_palette.bind(
                "<Button-3>",
                lambda event, menu=menus[btn_idx], button_id=btn_idx: self.context_menu(
                    event, menu
                ),
            )
    ...
    def render_keystone_shades_palette(self, keystone_colour: str, harmony_method: str):
        ...
        for _idx in range(num_shade_buttons):
        ...
        for btn_idx in range(len(shades_list)):
            btn_keystone_shade = ctk.CTkButton(
                master=self.frm_shades_palette,
            ...
            cbtk.CBtkToolTip(btn_keystone_shade, text="Right click for copy option.")
            ...
            menus[btn_idx].add_command(
                label="Copy",
                command=lambda button_id=btn_idx: self.copy_keystone_colour(
                    harmony_button_id=button_id
                ),
            )

            btn_keystone_shade.bind(
                "<Button-3>",
                lambda event, menu=menus[btn_idx], button_id=btn_idx: self.context_menu(
                    event, menu
                ),
    ...
    def copy_harmonics_to_palette(self):
        ...
        log.log_debug(
            log_text="Copy harmonics to theme colour palette",
            class_name="HarmonicsDialog",
        ...
        for idx in range(num_harmony_colours):
            ...
            if harmony_idx < num_harmony_colours - 1:
                ...
                harmony_idx = 0
        self.harmony_status_bar.set_status_text(
            status_text="Keystone and generated colours copied to palette."
        )
    ...
    def tag_keystone_colour_to_theme(self):
        ...
        harmony_method = self.opm_harmony_method.get()
        log.log_debug(
            log_text="Tag the keystone colour, and harmony method, to the theme",
            class_name="HarmonicsDialog",
            method_name="copy_harmonics_to_palette",
        )
        self.theme_json_data["provenance"]["keystone colour"] = keystone_colour
        self.theme_json_data["provenance"]["harmony method"] = harmony_method
        self.theme_json_data["provenance"]["harmony differential"] = (
            self.harmony_contrast_differential
        )
        self.master.json_state = "dirty"
        self.master.set_option_states()
        self.harmony_status_bar.set_status_text(
