...
import ctk_themio.utils.cbtk_kit as cbtk
import ctk_themio.utils.loggerutl as log
from ctk_themio.model.ctk_theme_builder import log_call
from ctk_themio.view.about import About
from ctk_themio.view.export_import import Exporter, Importer
from ctk_themio.view.geometry_dialog import GeometryDialog
from ctk_themio.view.harmonics_dialog import HarmonicsDialog
from ctk_themio.view.preferences import PreferencesDialog
from ctk_themio.view.provenance_dialog import ProvenanceDialog
from ctk_themio.view.theme_merger import ThemeMerger
...
class ControlPanel(ctk.CTk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.top_frame = "top"
        log.log_started(
            class_name="ControlPanel", supplementary_text="Theme Builder Control Panel started..."
        )
        ...
        if LISTENER_FILE.exists():
            try:
                ...
                log.log_critical(
                    f"ERROR: Could not remove listener semaphore file, {LISTENER_FILE}."
                )
        ...
        self.enable_tooltips = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="user_preference", preference_name="enable_tooltips"
        )

        self.confirm_cascade = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="user_preference", preference_name="confirm_cascade"
        )

        self.enable_palette_labels = pref.preference_setting(
            db_file_path=DB_FILE_PATH,
            scope="user_preference",
            preference_name="enable_palette_labels",
        )

        self.enable_single_click_paste = pref.preference_setting(
            db_file_path=DB_FILE_PATH,
            scope="user_preference",
            preference_name="enable_single_click_paste",
        )

        self.last_theme_on_start = pref.preference_setting(
            db_file_path=DB_FILE_PATH,
            scope="user_preference",
        ...
        if self.last_theme_on_start == "NO_DATA_FOUND":
            self.last_theme_on_start = pref.new_preference_dict(
                scope="user_preference",
                preference_name="last_theme_on_start",
                data_type="int",
                preference_value=0,
            ...
            pref.upsert_preference(
                db_file_path=DB_FILE_PATH, preference_row_dict=self.last_theme_on_start
            )

        self.control_panel_scaling_pct = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="scaling", preference_name="control_panel"
        )
        control_panel_scale = mod.scaling_float(scale_pct=self.control_panel_scaling_pct)
        ctk.set_widget_scaling(control_panel_scale)

        self.preview_panel_scaling_pct = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="scaling", preference_name="preview_panel"
        )

        self.qa_application_scaling_pct = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="scaling", preference_name="qa_application"
        )

        self.theme_json_dir = pref.preference_setting(
        ...
        if self.theme_json_dir == "NO_DATA_FOUND":
            ...
            theme_dir_row = pref.new_preference_dict(
                scope="user_preference",
        ...
        self.shade_adjust_differential = pref.preference_setting(
            db_file_path=DB_FILE_PATH,
            scope="user_preference",
            preference_name="shade_adjust_differential",
        )

        self.harmony_contrast_differential = pref.preference_setting(
            db_file_path=DB_FILE_PATH,
            scope="user_preference",
            preference_name="harmony_contrast_differential",
        )

        self.render_disabled = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="auto_save", preference_name="render_disabled"
        )

        self.theme_author = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="user_preference", preference_name="theme_author"
        )

        self.min_ctk_version = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="system", preference_name="min_ctk_version"
        )
        ...
        self.control_panel_mode = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="user_preference", preference_name="control_panel_mode"
        )
        self.TEMP_DIR = TEMP_DIR

        cbtk.str_mode_to_int(self.control_panel_mode)
        ...
        self.ctl_frame_high = cbtk.theme_property_color(
            theme_file_path=control_panel_theme_path,
            widget_type="CTkToplevel",
            widget_property="fg_color",
            mode=self.control_panel_mode,
        )

        self.ctl_text = cbtk.theme_property_color(
            theme_file_path=control_panel_theme_path,
        ...
        self.ctl_frame_border = cbtk.theme_property_color(
            theme_file_path=control_panel_theme_path,
        ...
        try:
            ...
            log.log_warning(
                'Preferred Control Panel, theme file not found. Falling back to "blue" theme.',
                class_name="ControlPanel",
        ...
        button_frame.grid(
            row=0, column=0, rowspan=3, columnspan=1, sticky="ns", padx=(5, 5), pady=(5, 5)
        )

        self.frm_geometry.grid(row=0, column=1, rowspan=1, sticky="nswe", padx=(0, 5), pady=(5, 5))
        self.frm_theme_palette.grid(
            row=1, column=1, rowspan=1, sticky="nswe", padx=(0, 5), pady=(5, 5)
        )
        self.frm_colour_edit_widgets.grid(
            row=2, column=1, rowspan=1, sticky="nswe", padx=(0, 5), pady=(5, 5)
        )
        self.status_bar = cbtk.CBtkStatusBar(master=self, status_text_life=30, use_grid=False)
        self.bind("<Configure>", self.status_bar.auto_size_status_bar)

        # Populate Frames
        self.lbl_title = ctk.CTkLabel(
            master=title_frame, text="Control Panel", font=HEADING4, anchor="w"
        )
        ...
        last_theme = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="auto_save", preference_name="selected_theme"
        )
        ...
        self.opm_theme = ctk.CTkOptionMenu(
            master=self.frm_button, values=initial_display, command=self.load_theme
        )
        ...
        mode = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="auto_save", preference_name="appearance_mode"
        )
        ...
        self.swt_frame_mode = ctk.CTkSwitch(
            self.frm_button,
            state=tk.DISABLED,
            text="Top Frame",
            onvalue="top",
        ...
        self.swt_render_disabled = ctk.CTkSwitch(
        ...
        if self.enable_tooltips:
            CTkToolTip(
                self.swt_render_disabled,
                wraplength=400,
                justify="left",
                border_width=1,
                padding=(10, 10),
                corner_radius=6,
                message="Enable this switch, to preview widget appearances in disabled mode.",
            )

        self.lbl_widget_view = ctk.CTkLabel(master=self.frm_button, text="Properties View:")
        ...
        self.opm_properties_view = ctk.CTkOptionMenu(
            master=self.frm_button,
        ...
        self.widget_categories = mod.all_widget_categories(DEFAULT_VIEW_WIDGET_ATTRIBUTES)
        self.opm_properties_filter = ctk.CTkOptionMenu(
            master=self.frm_button,
        ...
        self.btn_undo = ctk.CTkButton(
            master=button_frame, text="Undo", state=tk.DISABLED, command=self.undo_change
        )
        self.btn_undo.grid(row=15, column=0, padx=5, pady=(30, 5))

        self.btn_redo = ctk.CTkButton(
            master=button_frame, text="Redo", state=tk.DISABLED, command=self.redo_change
        )
        self.btn_redo.grid(row=17, column=0, padx=5, pady=(5, 5))

        self.btn_reset = ctk.CTkButton(
            master=button_frame, text="Reset", state=tk.DISABLED, command=self.reset_theme
        )
        self.btn_reset.grid(row=18, column=0, padx=5, pady=(5, 5))

        self.btn_create = ctk.CTkButton(
            master=button_frame, text="New Theme", command=self.create_theme
        )
        self.btn_create.grid(row=19, column=0, padx=5, pady=(30, 5))

        self.btn_sync_modes = ctk.CTkButton(
            master=button_frame,
            text="Sync Modes",
            state=tk.DISABLED,
        ...
        self.btn_sync_palette = ctk.CTkButton(
            master=button_frame,
            text="Sync Palette",
            state=tk.DISABLED,
        ...
        self.btn_save = ctk.CTkButton(
            master=button_frame, text="Save", state=tk.DISABLED, command=self.save_theme
        )
        self.btn_save.grid(row=22, column=0, padx=5, pady=(30, 5))

        self.btn_save_as = ctk.CTkButton(
            master=button_frame, text="Save As", state=tk.DISABLED, command=self.save_theme_as
        )
        self.btn_save_as.grid(row=25, column=0, padx=5, pady=(5, 5))

        self.btn_delete = ctk.CTkButton(
            master=button_frame, text="Delete", state=tk.DISABLED, command=self.delete_theme
        )
        ...
        if int(base_min_version) > int(base_ctk_version):
            message = (
                f"The version of CustomTkinter ({ctk_version}), on your system, is incompatible. Please upgrade "
                f"to CustomTkinter {self.min_ctk_version} or later."
            )
            log.log_warning(log_text=message, class_name="ControlPanel", method_name="__init__()")
            confirm = CTkMessagebox(
                master=self, title="Please Upgrade CustomTkinter", message=message, option_1="OK"
            )
            if confirm.get() == "OK":
                exit(1)
    ...
    def flip_appearance_modes(self):
        if self.json_state == "dirty":
            confirm = CTkMessagebox(
                master=self,
                title="Acknowledge",
                message="You have unsaved changes. You must Save or Reset any changes, "
                "before using the Flip Modes option.",
                options=["OK"],
            )
        ...
        log.log_debug(
            log_text="Flip appearance modes",
            class_name="ControlPanel",
            method_name="flip_appearance_modes",
        )

        confirm = CTkMessagebox(
            master=self,
            title="Confirm Action",
            message="This will swap around, all of the theme appearance mode colours, between "
            "Light mode and Dark mode. Are you sure you wish to proceed?",
            options=["Yes", "No"],
        )
        response = confirm.get()
        if response == "No":
    ...
    def set_widget_colour(self, widget_property, new_colour):
        ...
        if not cbtk.valid_colour(new_colour):
            self.status_bar.set_status_text(
                status_text="Paste action ignored - not a valid colour code."
            )
            return
        ...
        if prev_colour != new_colour and widget_property in mod.FORCE_COLOR_REFRESH_PROPERTIES:
            ...
            change_vector = mod.PropertyVector(
                command_type="colour",
    ...
    def create_menu(self):
        ...
        cbtk.get_color_from_name(widget_type="CTkLabel", widget_property="text_color")
        cbtk.get_color_from_name(widget_type="CTkToplevel", widget_property="fg_color")
        ...
        self.file_menu.add_command(
            label="Launch QA App", command=self.launch_qa_app, state=tk.DISABLED
        )
        self.file_menu.add_separator()
        self.file_menu.add_command(
            label="Export Theme", command=self.launch_export_dialog, state=tk.NORMAL
        )
        self.file_menu.add_command(
            label="Import Theme", command=self.launch_import_dialog, state=tk.NORMAL
        )
        self.file_menu.add_separator()
        ...
        self.tools_menu.add_command(
            label="Colour Harmonics", command=self.launch_harmony_dialog, state=tk.DISABLED
        )
    ...
    def launch_theme_merger(self):
        log.log_debug(
            log_text="Launching Theme Merger",
            class_name="ControlPanel",
            method_name="launch_theme_merger",
        )
    ...
    def copy_property_colour(self, event=None, widget_property=None):
        ...
        try:
            ...
            log.log_error(
                f"Key Error on shade copy: Widget Property = {widget_property}",
                class_name="ControlPanel",
                method_name="copy_property_colour()",
            )
        if colour:
            ...
            self.status_bar.set_status_text(status_text=f"Copy for {colour} failed!")

    @log_call
    def about(self):
        log.log_debug(
            log_text="Launching About dialogue", class_name="ControlPanel", method_name="about"
        )
        About()
    ...
    def launch_export_dialog(self):
        log.log_debug(
            log_text="Launching export dialogue",
            class_name="ControlPanel",
            method_name="launch_export_dialog",
        )
        Exporter(master=self)
    ...
    def launch_preferences_dialog(self):
        log.log_debug(
            log_text="Launching preferences dialogue",
            class_name="ControlPanel",
            method_name="launch_preferences_dialog",
        )
        ...
        if listener_port != self.listener_port:
            log.log_debug(
                log_text="Listener port has changed, reloading preview",
                class_name="ControlPanel",
                method_name="launch_preferences_dialog",
            )
            self.reload_preview()
    ...
    def launch_provenance_dialog(self):
        log.log_debug(
            log_text="Launching provenance dialogue",
            class_name="ControlPanel",
            method_name="launch_provenance_dialog",
        )
        ...
        provenance_dialog.modify_property(
            property_name="last_modified_date", value=last_modified_date
        )
    ...
    def launch_qa_app(self):
        log.log_debug(
            log_text="Launching QA app", class_name="ControlPanel", method_name="launch_qa_app"
        )
    ...
    def load_preferences(self):
        ...
        self.control_panel_scaling = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="scaling", preference_name="control_panel"
        )

        self.preview_panel_scaling = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="scaling", preference_name="preview_panel"
        )

        self.qa_application_scaling = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="scaling", preference_name="qa_application"
        )

        self.listener_port = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="user_preference", preference_name="listener_port"
        )
    ...
    def sync_palette_mode_colours(self):
        ...
        log.log_debug(
            log_text="Sync palette mode colours",
            class_name="ControlPanel",
            method_name="sync_palette_mode_colours",
        )
        for entry_id, _button in enumerate(self.theme_palette_tiles):
    ...
    def stash_theme_palette(self, mode):
        ...
        log.log_debug(
            log_text="Stash the theme colour palette colours to disk",
            class_name="ControlPanel",
            method_name="stash_theme_palette",
        )
        for entry_id, _button in enumerate(self.theme_palette_tiles):
            fg_colour = self.theme_palette_tiles[entry_id].cget("fg_color")
    ...
    def save_theme_palette(self, theme_name: str = None):
        ...
        log.log_debug(
            log_text=f"Save the colour palette colours back to disk; palette_json_file: {palette_json_file}",
            class_name="ControlPanel",
            method_name="save_theme_palette",
        )
    ...
    def render_geometry_buttons(self):
        ...
        log.log_debug(
            log_text="Set up the geometry buttons near the top of the Control Panel",
            class_name="ControlPanel",
            method_name="render_geometry_buttons",
        )
        ...
        lbl_header = ctk.CTkLabel(
            master=self.frm_geometry, text="Widget Geometry", font=mod.HEADING4
        )
        lbl_header.grid(row=0, column=0, sticky="w", columnspan=12, pady=(5, 0), padx=10)
        row = 1
        column = 0
        btn_geo_button = ctk.CTkButton(
            master=self.frm_geometry,
            text="Button",
        ...
        btn_geo_button.grid(row=row, column=column, padx=pad_x, pady=pad_y)
        column += 1

        btn_geo_checkbox = ctk.CTkButton(
            master=self.frm_geometry,
            text="CheckBox",
            height=button_height,
            width=button_width,
        ...
        btn_geo_checkbox.grid(row=row, column=column, padx=pad_x, pady=pad_y)
        column += 1

        btn_geo_combobox = ctk.CTkButton(
            master=self.frm_geometry,
            text="ComboBox",
            height=button_height,
            width=button_width,
        ...
        btn_geo_combobox.grid(row=row, column=column, padx=pad_x, pady=pad_y)
        column += 1

        btn_geo_entry = ctk.CTkButton(
            master=self.frm_geometry,
            text="Entry",
            height=button_height,
            width=button_width,
        ...
        btn_geo_entry.grid(row=row, column=column, padx=pad_x, pady=pad_y)
        column += 1

        btn_geo_frame = ctk.CTkButton(
            master=self.frm_geometry,
            text="Frame",
            height=button_height,
            width=button_width,
        ...
        btn_geo_frame.grid(row=row, column=column, padx=pad_x, pady=pad_y)
        column += 1

        btn_geo_label = ctk.CTkButton(
            master=self.frm_geometry,
            text="Label",
            height=button_height,
            width=button_width,
        ...
        btn_geo_label.grid(row=row, column=column, padx=pad_x, pady=pad_y)

        column += 1
        btn_geo_option_menu = ctk.CTkButton(
            master=self.frm_geometry,
            text="OptionMenu",
            height=button_height,
            width=button_width,
        ...
        btn_geo_option_menu.grid(row=row, column=column, padx=pad_x, pady=pad_y)

        row += 1
        column = 0

        btn_geo_progress = ctk.CTkButton(
            master=self.frm_geometry,
            text="ProgressBar",
            height=button_height,
            width=button_width,
        ...
        btn_geo_progress.grid(row=row, column=column, padx=pad_x, pady=pad_y)

        column += 1

        btn_geo_radio = ctk.CTkButton(
            master=self.frm_geometry,
            text="RadioButton",
            height=button_height,
            width=button_width,
        ...
        btn_geo_radio.grid(row=row, column=column, padx=pad_x, pady=pad_y)

        column += 1

        btn_geo_scrollbar = ctk.CTkButton(
            master=self.frm_geometry,
            text="SegButton",
            height=button_height,
            width=button_width,
        ...
        btn_geo_scrollbar.grid(row=row, column=column, padx=pad_x, pady=pad_y)
        ...
        btn_geo_slider.grid(row=row, column=column, padx=pad_x, pady=pad_y)
        column += 1

        btn_geo_switch = ctk.CTkButton(
            master=self.frm_geometry,
            text="Switch",
            height=button_height,
            width=button_width,
        ...
        btn_geo_switch.grid(row=row, column=column, padx=pad_x, pady=pad_y)
        column += 1
    ...
    def launch_widget_geometry(self, widget_type):
        log.log_debug(
            log_text="Launch widget geometry dialogue",
            class_name="ControlPanel",
            method_name="launch_widget_geometry",
        )
        geometry_dialog = GeometryDialog(
            master=self,
    ...
    def set_option_states(self):
        ...
        log.log_debug(
            log_text="Set the button and menu option states",
            class_name="ControlPanel",
            method_name="set_option_states",
        )
        ...
        if self.theme:
            log.log_debug(
                log_text="Theme already opened, so state = tk.NORMAL",
                class_name="ControlPanel",
                method_name="set_option_states",
            )
            tk_state = tk.NORMAL
        ...
        if self.harmony_palette_running:
            ...
            not self.harmony_palette_running and self.theme and "provenance" in self.theme_json_data
        ):
    ...
    def switch_preview_appearance_mode(self, event="event"):
        ...
        log.log_debug(
            log_text=f"Switch appearance mode: {preview_appearance_mode}",
            class_name="ControlPanel",
            method_name="switch_preview_appearance_mode",
        )
        ...
        if not mod.update_preference_value(
            db_file_path=DB_FILE_PATH,
            scope="auto_save",
            preference_name="appearance_mode",
            preference_value=self.appearance_mode,
        ):
            log.log_warning(log_text="Row miss: on update of auto save of selected theme.")
    ...
    def render_theme_palette(self):
        ...
        self.lbl_palette_header = ctk.CTkLabel(
            master=self.frm_theme_palette,
            text=f"Theme Palette ({preview_appearance_mode})",
            font=HEADING4,
        )

        self.lbl_palette_header.grid(
            row=0, column=0, sticky="w", columnspan=12, pady=(5, 0), padx=10
        )
        ...
        for palette_id, tile_dict in enumerate(palette_entries):
            ...
            context_menu.add_command(
                label="Copy",
                command=lambda button_id=entry_id: self.copy_palette_colour(
            ...
            context_menu.add_command(
                label="Paste",
                command=lambda button_id=entry_id: self.paste_palette_colour(
                    event=None, palette_button_id=button_id
                ),
            )
            ...
            shade_up_menu.add_command(
                label="Lighter",
                state="normal",
                command=lambda button_id=entry_id, pal_button=btn_colour_tile: (
                    self.lighten_palette_tile(
                        palette_button=pal_button,
                        palette_button_id=button_id,
            ...
            shade_down_menu.add_command(
                label="Darker",
                state="normal",
                command=lambda button_id=entry_id, pal_button=btn_colour_tile: (
                    self.darken_palette_tile(
                        palette_button=pal_button,
                        palette_button_id=button_id,
                        shade_step=self.shade_adjust_differential,
                        multiplier=1,
            ...
            if self.enable_single_click_paste:
                btn_colour_tile.bind(
                    "<Button-1>",
                    lambda event, button_id=entry_id: self.paste_palette_colour(event, button_id),
                )

            btn_colour_tile.bind(
                "<Button-3>",
                lambda event, menu=menus[entry_id], button_id=entry_id: self.context_menu(
                    event, menu
                ),
            )
    ...
    def cascade_colour(self, palette_id):
        ...
        for _property in cascade_dict_list:
            ...
            self.paste_colour(
                event=None, widget_property=_widget_property, property_colour=property_colour
            )
    ...
    def toggle_frame_mode(self):
        ...
        if frame_mode == "top":
            log.log_debug(
                log_text="Frame = top", class_name="ControlPanel", method_name="toggle_frame_mode"
            )
            mod.send_command_json(command_type="program", command="render_top_frame")
        else:
            log.log_debug(
                log_text="Frame = base", class_name="ControlPanel", method_name="toggle_frame_mode"
            )
    ...
    def undo_change(self):
        undo_text, _command_type, _widget_type, _widget_property, _property_value = (
            self.command_stack.undo_command()
        )
        _command = _widget_type
        if _command_type == "colour":
            self.update_rendered_widget(
                widget_type=_widget_type,
                widget_property=_widget_property,
        ...
        self.set_option_states()
        self.status_bar.set_status_text(status_text=undo_text)

    @log_call
    def redo_change(self):
        redo_text, _command_type, _widget_type, _widget_property, _property_value = (
            self.command_stack.redo_command()
        )
        _command = _widget_type
        if _command_type == "colour":
            self.update_rendered_widget(
        ...
        self.set_option_states()
        self.status_bar.set_status_text(status_text=redo_text)
    ...
    def load_theme(self, event=None, reload_preview: bool = True):
        ...
        if not mod.update_preference_value(
            ...
            log.log_warning(log_text="Row miss: on update of auto save of selected theme.")
        self.status_bar.set_status_text(
            status_text_life=30, status_text=f"Theme file, {self.theme_file}, loaded. "
        )
        self.json_state = "clean"
        self.set_option_states()
    ...
    def save_harmonics_geometry(self):
        ...
        geometry_row = pref.preference_row(
            db_file_path=DB_FILE_PATH, scope="window_geometry", preference_name="harmonics_panel"
        )
    ...
    def launch_harmony_dialog(self):
        HarmonicsDialog(theme_name=self.theme, theme_json_data=self.theme_json_data)

    @log_call
    def darken_palette_tile(
        self,
        ...
        darker_shade = cbtk.shade_down(
            color=widget_colour, differential=shade_step, multiplier=multiplier
        )
    ...
    @log_call
    def lighten_widget_property_shade(
        self,
        property_widget: ctk.CTkButton,
        widget_property: str,
        shade_step: int,
        multiplier: int = 1,
    ):
    ...
    @log_call
    def darken_widget_property_shade(
        self,
        property_widget: ctk.CTkButton,
        widget_property: str,
        shade_step: int,
    ...
    def create_theme(self):
        ...
        if new_theme:
            if not mod.valid_theme_file_name(new_theme):
                self.status_bar.set_status_text(
                    status_text="The entered theme name contains disallowed characters - allowed: alphanumeric, "
                    "underscores & ()"
                )
                return
            ...
            if new_theme_path.exists():
                self.status_bar.set_status_text(
                    status_text=f"Theme {new_theme} already exists - please choose another name!"
                )
                return
            ...
            self.theme_json_data["provenance"]["created with"] = (
                f"CTk Theme Builder v{mod.app_version()}"
            )
            # current dateTime
            now = datetime.now()
            # convert to string
    ...
    def delete_theme(self):
        ...
        self.btn_save.configure(state=tk.DISABLED)
        self.status_bar.set_status_text(
            status_text_life=30, status_text=f'Theme, "{self.theme}", has been deleted. '
        )
        self.json_state = "clean"
    ...
    def save_theme_as(self):
        ...
        if new_theme:
            if not mod.valid_theme_file_name(new_theme):
                self.status_bar.set_status_text(
                    status_text="The entered theme name contains prohibited characters "
                    "- allowed: alphanumeric, underscores & ()"
                )
                return
    ...
    def paste_colour(self, event, widget_property, property_colour: str = None):
        ...
        log.log_debug(
            log_text=f"Paste colour: widget_property={widget_property}; property_colour={property_colour}",
            class_name="ControlPanel",
            method_name="paste_colour",
        )
        ...
        if not cbtk.valid_colour(new_colour):
            self.status_bar.set_status_text(
                status_text="Attempted paste of non colour code text - ignored."
            )
            return
        ...
        # region, so we need to check, the selected view, may not include the widget.
        if self.widgets[widget_property]["tile"].winfo_exists():
            self.widgets[widget_property]["tile"].configure(
                fg_color=new_colour, hover_color=hover_colour
            )
    ...
    def paste_palette_colour(self, event, palette_button_id):
        ...
        if not cbtk.valid_colour(new_colour):
            self.status_bar.set_status_text(
                status_text="Attempted paste of non colour code - pasted text ignored."
            )
            return
        old_colour = self.theme_palette_tiles[palette_button_id].cget("fg_color")
        change_vector = mod.PropertyVector(
            command_type="palette_colour",
            command="null",
    ...
    def set_palette_colour(self, palette_button_id, colour):
        ...
        hover_colour = cbtk.contrast_colour(colour)
        self.theme_palette_tiles[palette_button_id].configure(
            fg_color=colour, hover_color=hover_colour
        )
        self.json_state = "dirty"
        self.set_option_states()
    ...
    def property_colour_picker(self, event, widget_property):
        prev_colour = self.widgets[widget_property]["colour"]
        new_colour = askcolor(
            master=self, title="Pick colour for : " + widget_property, initialcolor=prev_colour
        )
    ...
    def palette_colour_picker(self, palette_button_id):
        ...
        log.log_debug(
            log_text=f"prev_colour={prev_colour}; new_colour={new_colour}",
            class_name="ControlPanel",
            method_name="property_colour_picker",
        )
        if new_colour[1] is not None:
            new_colour = new_colour[1]
            old_colour = self.theme_palette_tiles[palette_button_id].cget("fg_color")
            ...
            self.theme_palette_tiles[palette_button_id].configure(
                fg_color=new_colour, hover_color=new_colour
            )
            self.status_bar.set_status_text(
                status_text=f"Colour {new_colour} assigned to palette entry {palette_button_id + 1}."
            )
    ...
    def copy_palette_colour(self, event=None, palette_button_id=None, shade_copy=False):
        log.log_debug(
            log_text=f"Copy palette colour: palette_button_id={palette_button_id}; shade_copy={shade_copy}",
            class_name="ControlPanel",
            method_name="copy_palette_colour",
        )
    ...
    def render_widget_properties(self, dummy=None):
        ...
        log.log_debug(
            log_text="Render the widget properties, within the control panel",
            class_name="ControlPanel",
            method_name="render_widget_properties",
        )
        ...
        self.lbl_header = ctk.CTkLabel(
            master=widget_frame, text=f" Colour Mappings: {view}", font=HEADING3
        )

        self.lbl_header.grid(row=0, column=0, sticky="w", columnspan=12, pady=(5, 0), padx=0)
        ...
        member_gen = mod.widget_member(
            widget_entries=rendered_widgets, filter_list=self.filter_list
        )
        for row in range(1, 30):
            for column_base in range(0, 3):
                ...
                if self.enable_single_click_paste and colour_value != "transparent":
                    self.widgets[key]["tile"].bind(
                        "<Button-1>",
                        lambda event, wgt_property=key: self.paste_colour(event, wgt_property),
                    )
                ...
                if self.enable_tooltips:
                    if colour_value != "transparent":
                        cbtk.CBtkToolTip(
                            btn_property, text="Right click for colour & clipboard functions."
                        )
                    else:
                        cbtk.CBtkToolTip(
                            btn_property,
                ...
                if colour_value != "transparent":
                    ...
                    context_menu.add_command(
                        label="Copy",
                        command=lambda wgt_property=key: self.copy_property_colour(
                            event=None, widget_property=wgt_property
                        ),
                    )

                    context_menu.add_command(
                        label="Paste",
                        command=lambda wgt_property=key: self.paste_colour(
                            event=None, widget_property=wgt_property
                        ),
                    )
                    context_menu.add_separator()
                    ...
                    shade_up_menu.add_command(
                        label="Lighter",
                        state="normal",
                        command=lambda widget=btn_property, wgt_type=widget_type, wgt_property=key: (
                            self.lighten_widget_property_shade(
                    ...
                    shade_down_menu.add_command(
                        ...
                        state="normal",
                        command=lambda widget=btn_property, wgt_type=widget_type, wgt_property=key: (
                            self.darken_widget_property_shade(
                                property_widget=widget,
                                widget_property=wgt_property,
                                shade_step=self.shade_adjust_differential,
                                multiplier=1,
                    ...
                    context_menu.add_command(
                        label="Colour Picker",
                        command=lambda wgt_property=key: self.property_colour_picker(
                            event=None, widget_property=wgt_property
                        ),
                    )
                    self.widgets[key]["tile"].bind(
                        "<Button-3>",
                        lambda event, menu=context_menu, button_id=btn_idx: self.context_menu(
                            event, menu
                        ),
                    )
    ...
    def update_rendered_widget(self, widget_type, widget_property, property_value):
        ...
        mod.PropertyVector.display_property(
            widget_type=widget_type, widget_property=widget_property
        )
        log.log_debug(
            log_text="Update rendered widget",
            class_name="ControlPanel",
            method_name="update_rendered_widget",
        )
        for widget_record in self.widgets.values():
            ...
            appearance_mode_index = cbtk.str_mode_to_int(self.appearance_mode)
            self.theme_json_data[widget_type][widget_property][appearance_mode_index] = (
                property_value
            )
    ...
    def sync_appearance_mode(self):
        ...
        if confirm.get() == "Cancel":
            return
        log.log_debug(
            log_text=f"Sync (copy) appearance mode: from {current_mode} to {to_mode_description}",
            class_name="ControlPanel",
            method_name="sync_appearance_mode",
        )
        counter = 0
        for property_id in self.filter_list:
            ...
            if str(self.theme_json_data[json_widget_type][widget_property]) == "transparent":
                continue
            try:
                self.theme_json_data[json_widget_type][widget_property][to_mode] = self.widgets[
                    property_id
                ]["colour"]
                counter += 1
            except TypeError:
                log.log_critical(
                    log_text=f"TypeError on: {widget_type} / {widget_property}",
                    class_name="ControlPanel",
                    method_name="sync_appearance_mode()",
                )
                log.log_supplementary(
                    f"Current setting: {str(self.theme_json_data[widget_type][widget_property])}"
                )
                log.log_supplementary(
                    f"Attempt setting: {str(self.widgets[property_id]['colour'])}"
                )
                raise
            except KeyError:
                log.log_critical(
                    f"KeyError on: {widget_type} / {widget_property}",
                    class_name="ControlPanel",
                    method_name="sync_appearance_mode()",
                )
                log.log_supplementary(
                ...
                raise
        log.log_info(
            f"{counter} properties synchronised.",
            class_name="ControlPanel",
            method_name="sync_appearance_mode()",
        )
        self.json_state = "dirty"
    ...
    def sync_theme_palette(self):
        ...
        if confirm.get() == "Cancel":
            return
        log.log_debug(
            log_text=f"Sync (copy) theme palette mode: from {current_mode} to {to_mode_description}",
            class_name="ControlPanel",
            method_name="sync_theme_palette",
        )
    ...
    def refresh_preview(self, set_scaling: bool = True):
        ...
        log.log_debug(
            log_text=f"Refresh preview set_scaling={set_scaling}",
            class_name="ControlPanel",
            method_name="refresh_preview",
        )
        self.update_wip_file()
        mod.send_command_json(
            command_type="program", command="refresh", parameters=[self.appearance_mode]
        )
    ...
    def reload_preview(self):
        ...
        log.log_debug(
            log_text="Reload preview panel", class_name="ControlPanel", method_name="reload_preview"
        )
        if self.process:
            mod.send_command_json(command_type="program", command="quit", parameters=None)
        ...
        if self.tk_render_disabled.get():
            mod.send_command_json(
                command_type="program", command="render_preview_disabled", parameters=None
            )
        else:
            mod.send_command_json(
                command_type="program", command="render_preview_enabled", parameters=None
            )
    ...
    def close_panels(self, event=None):
        if self.json_state == "dirty":
            confirm = CTkMessagebox(
                master=self,
                title="Confirm Action",
                message="You have unsaved changes. Do you wish to save these before quitting?",
            ...
            if response == "Cancel":
                ...
                self.save_theme()
        log.log_debug(
            log_text="Close panels", class_name="ControlPanel", method_name="close_panels"
        )
        if self.qa_launched:
        ...
        log.log_complete(
            class_name="ControlPanel", supplementary_text="Theme Builder Control Panel exiting"
        )
        self.destroy()
    ...
    def launch_preview(self):
        log.log_info(
            log_text="Launching preview panel",
            class_name="ControlPanel",
        ...
        if self.process is None:
            ...
            log.log_debug(
                log_text=f"Launching designer: {program}",
                class_name="ControlPanel",
                method_name="launch_preview",
            )
            self.process = sp.Popen(program)
            ...
            while not listener_started:
                log.log_debug(
                    log_text=f"Checking for a listener - try {sleep_count + 1}...",
                    class_name="ControlPanel",
                    method_name="launch_preview",
                )
                sleep_count += 1
                ...
                if sleep_count > 80:
                    ...
                    log.log_critical(
                        "ERROR: Waited too long for listener!",
    ...
    def restore_controller_geometry(self):
        log.log_debug(
            log_text="Restore Control Panel geometry",
            class_name="ControlPanel",
            method_name="restore_controller_geometry",
        )
        controller_geometry = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="window_geometry", preference_name="control_panel"
        )
    ...
    def save_controller_geometry(self):
        ...
        log.log_debug(
            log_text="Save Control Panel geometry",
            class_name="ControlPanel",
            method_name="restore_controller_geometry",
        )
        geometry_row = pref.preference_row(
    ...
    @log_call
    def save_theme(self):
        log.log_debug(log_text="Save theme", class_name="ControlPanel", method_name="save_theme")
        if "provenance" in self.theme_json_data:
            # current dateTime
            now = datetime.now()
            # convert to string
        ...
        self.status_bar.set_status_text(
            status_text=f"Theme file, {theme_file_name}, saved successfully!"
        )
        self.command_stack.reset_stacks()
        self.set_option_states()
