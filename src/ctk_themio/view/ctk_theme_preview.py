...
class PreviewPanel:
    ...
    def __init__(self, theme_file: str, appearance_mode: str = "Dark"):
        log.log_started(
            class_name="PreviewPanel", supplementary_text="Theme Builder Preview Panel launched"
        )
        ...
        self._enable_tooltips = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="user_preference", preference_name="enable_tooltips"
        )

        self._render_disabled = pref.preference_setting(
        ...
        self.img_selected = cbtk.load_image(
            light_image=APP_IMAGES / "colour_wheel.png", image_size=(10, 10)
        )
    ...
    def render_preview_frames(self):
        ...
        self._ctl_frame_fg_color = cbtk.theme_property_color(
            theme_file_path=self.control_panel_theme_path,
            widget_type="CTkFrame",
            widget_property="fg_color",
            mode=control_panel_mode,
        )

        self._ctl_frame_top_fg_color = cbtk.theme_property_color(
            theme_file_path=self.control_panel_theme_path,
            widget_type="CTkFrame",
        ...
        self._ctl_frame_border = cbtk.theme_property_color(
            theme_file_path=self.control_panel_theme_path,
            widget_type="CTkFrame",
        ...
        self._ctl_text = cbtk.get_color_from_name(
            widget_type="CTkLabel", widget_property="text_color"
        )

        self._ctl_label = cbtk.theme_property_color(
            theme_file_path=self.control_panel_theme_path,
        ...
        self.frm_preview_base.grid(
            row=0, column=0, ipadx=ipadx, ipady=ipady, padx=10, pady=(10, 20), sticky="nsew"
        )
        ...
        self.frm_preview_top.grid(
            row=1, column=0, ipadx=ipadx, ipady=ipady, padx=10, pady=(10, 10), sticky="nsew"
        )
    ...
    def _render_frame_top_preview(self):
        ...
        theme_name = cbtk.theme_provenence_attribute(
            theme_file_path=self._theme_file,
        ...
        self.lbl_preview_heading = ctk.CTkLabel(
            master=self.frm_preview_top, text="Frame (Top) Preview", font=mod.HEADING4, anchor="w"
        )
        ...
        self.combobox_1 = ctk.CTkComboBox(
            widget_frame, values=["Option 1", "Option 2", "Option 42..."]
        )
        ...
        # CTkButton
        eye_con = ctk.CTkImage(
            light_image=Image.open(APP_IMAGES / "eye_lm.png"),
        ...
        if self._enable_tooltips:
            self.button_1_tooltip = CTkToolTip(
                self.button_1,
                justify="left",
                padding=(10, 10),
                border_width=1,
        ...
        self.button_2 = ctk.CTkButton(
            master=widget_frame, image=eye_con, border_width=second_border_width
        )
        self.button_2.grid(row=5, column=2, padx=pad_x, pady=pad_y)

        if self._enable_tooltips:
            self.button_2_tooltip = CTkToolTip(
                self.button_2,
        ...
        self.radiobutton_1 = ctk.CTkRadioButton(
            master=widget_frame, variable=self.radiobutton_var, value=1, text="CTkRadioButton"
        )
        ...
        self.radiobutton_2 = ctk.CTkRadioButton(
            master=widget_frame, variable=self.radiobutton_var, value=2
        )
        ...
        self.entry_1 = ctk.CTkEntry(
            master=widget_frame, placeholder_text="CTkEntry", border_width=entry_border_width
        )
        self.entry_1.grid(row=4, column=0, padx=pad_x, pady=pad_y)
        if self._enable_tooltips:
            self.entry_1_tooltip = CTkToolTip(
                self.entry_1,
                justify="left",
                border_width=1,
                padding=(10, 10),
        ...
        self.entry_2 = ctk.CTkEntry(
            master=widget_frame, border_width=second_border_width, placeholder_text="CTkEntry2"
        )
        self.entry_2.grid(row=5, column=0, padx=pad_x, pady=pad_y)
        if self._enable_tooltips:
            self.entry_2_tooltip = CTkToolTip(
                self.entry_2,
                justify="left",
                padding=(10, 10),
                border_width=1,
        ...
        self.scrollable_frame = ctk.CTkScrollableFrame(
            master=widget_frame, label_text="CTkScrollableFrame", width=160, height=200
        )
        ...
        self.scrollable_label = ctk.CTkLabel(
            master=self.scrollable_frame, text="Bozzy bear woz here...\n\n" * 20
        )
        ...
        self.label_tab_3 = ctk.CTkLabel(
            self.tabview.tab("Tab 3"),
            justify="left",
    ...
    def render_top_frame(self):
        ...
        border_width = cbtk.theme_property(
            theme_file_path=self._theme_file, widget_type="CTkFrame", widget_property="border_width"
        )
        self.frm_preview_top.configure(
            fg_color=self._ctl_frame_top_fg_color, border_width=border_width
        )
    ...
    def _restore_preview_geometry(self):
        panel_geometry = pref.preference_setting(
            db_file_path=DB_FILE_PATH, scope="window_geometry", preference_name="preview_panel"
        )
    ...
    def exec_program_command(self):
        ...
        if command == "quit":
            ...
            log.log_debug(
                log_text="Preview panel received quit command",
            ...
            log.log_complete(
                class_name="PreviewPanel", supplementary_text="Theme Builder Preview Panel closed"
            )
            exit(0)
        if command == "refresh":
            log.log_debug(
                log_text="Preview panel received refresh command",
                class_name="PreviewPanel",
                method_name="exec_program_command",
            )
            ctk.set_default_color_theme(self._theme_file)
        ...
        if command == "set_appearance_mode":
            log.log_debug(
                log_text="Preview panel received set_appearance_mode command",
                class_name="PreviewPanel",
                method_name="exec_program_command",
            )
            mode = parameters[0]
        ...
        if command == "set_widget_scaling":
            log.log_debug(
                log_text="Preview panel received set_widget_scaling command",
                class_name="PreviewPanel",
                method_name="exec_program_command",
            )
            ...
            log.log_debug(
                log_text="Preview panel received render_preview_disabled command",
                class_name="PreviewPanel",
                method_name="exec_program_command",
            )
            self._render_preview_disabled()
            ...
            log.log_debug(
                log_text="Preview panel received render_preview_enabled command",
                class_name="PreviewPanel",
                method_name="exec_program_command",
            )
            self._render_preview_enabled()
            ...
            log.log_debug(
                log_text="Preview panel received render_top_frame command",
                class_name="PreviewPanel",
                method_name="exec_program_command",
            )
            ...
            log.log_debug(
                log_text="Preview panel received render_base_frame command",
                class_name="PreviewPanel",
                method_name="exec_program_command",
            )
    ...
    def _exec_colour_command(self):
        ...
        if command == "update_widget_colour":
            ...
            log.log_error(
                log_text=f"Unrecognised method request: {command}",
                class_name="PreviewPanel",
                method_name="exec_program_command",
            )
    ...
    def _exec_geometry_command(self):
        ...
        if command != "update_widget_geometry":
            log.log_error(
                log_text=f"ERROR: Unrecognised method request: {command}",
                class_name="PreviewPanel",
        ...
        if widget_type == "CTkButton" and widget_property == "border_width":
            ...
            if self._enable_tooltips:
                self.button_1_tooltip.configure(
                    message=f"CTkButton - with default border setting of {button_border_width}"
                )
            ...
            if self._enable_tooltips:
                self.button_2_tooltip.configure(
                    message=f"TkButton - with border setting of {second_border_width}"
                )
            ...
            if self._enable_tooltips:
                self.entry_1_tooltip.configure(
                    message=f"CTkEntry - with default border setting of {entry_border_width}"
                )
            ...
            if self._enable_tooltips:
                self.entry_2_tooltip.configure(
                    message=f"CTkEntry - with border setting of {second_border_width}"
                )
    ...
    def _method_listener(self):
        ...
        log.log_info(
            log_text=f"Starting method listener on port {METHOD_LISTENER_PORT}...",
            class_name="PreviewPanel",
        ...
        try:
            ...
            log.log_critical(
                "Preview Panel, socket bind error.",
                class_name="PreviewPanel",
                method_name="_method_listener",
            )

            log.log_supplementary(
                "Ensure that no other instances of CTk Theme Builder "
        ...
        log.log_debug(
            log_text="Preview Panel: Checking for listener file...",
            class_name="PreviewPanel",
            method_name="_method_listener",
        )
        if not LISTENER_FILE.exists():
            log.log_debug(
                "Preview Panel: Listener file missing, creating...",
                class_name="PreviewPanel",
                method_name="_method_listener",
            )
            ...
            log.log_info(
                "Method listener successfully started",
                class_name="PreviewPanel",
                method_name="_method_listener",
            )
        while True:
            log.log_debug(
                log_text="Waiting for a client request...",
                class_name="PreviewPanel",
                method_name="_method_listener",
            )
            conn, address = server.accept()
            client_thread = threading.Thread(
                target=self._handle_client, args=(conn, address), daemon=True
            )
            client_thread.start()
    ...
    def start_method_listener(self):
        ...
        if listener_status == -1:
            ...
            log.log_critical(
                log_text=f"The listener failed to bind to port {METHOD_LISTENER_PORT}\n\n",
                class_name="PreviewPanel",
                method_name="start_method_listener",
            )
            log.log_supplementary(
                f"Ensure that only one instance of {__title__} is running and that no "
                f"other process is using the port."
    ...
    def update_widget_colour(self, widget_type, widget_property, widget_colour):
        ...
        elif widget_type == "CTkScrollbar" and widget_property in (
            "fg_color",
            "button_color",
            "button_hover_color",
        ):
