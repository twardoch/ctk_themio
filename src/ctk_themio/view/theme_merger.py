...
class ThemeMerger(ctk.CTkToplevel):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.opm_primary_theme = ctk.CTkOptionMenu(
            master=frm_widgets, variable=self.tk_primary_theme, values=mod.user_themes_list()
        )
        self.opm_primary_theme.grid(
            row=widget_start_row, column=1, padx=(0, 10), pady=(20, 5), sticky="w"
        )
        ...
        rdo_primary_light = ctk.CTkRadioButton(
            master=frm_widgets, text="Light", variable=self.tk_primary_mode, value="Light"
        )
        rdo_primary_light.grid(row=widget_start_row, column=3, pady=(20, 5), sticky="w")
        widget_start_row += 1

        rdo_primary_dark = ctk.CTkRadioButton(
            master=frm_widgets, text="Dark", variable=self.tk_primary_mode, value="Dark"
        )
        ...
        widget_start_row = 0
        lbl_primary_mapped_mode = ctk.CTkLabel(
            master=frm_widgets, text="Map primary mode to", justify="right"
        )
        lbl_primary_mapped_mode.grid(
            row=widget_start_row, column=4, padx=5, pady=(20, 5), sticky="e"
        )
        if self.enable_tooltips:
            CTkToolTip(
                lbl_primary_mapped_mode,
                wraplength=250,
                justify="left",
                border_width=1,
                padding=(10, 10),
        ...
        rdo_primary_mapped_light = ctk.CTkRadioButton(
            master=frm_widgets, text="Light", variable=self.tk_primary_mapped_mode, value="Light"
        )
        rdo_primary_mapped_light.grid(row=widget_start_row, column=5, pady=(20, 5), sticky="w")
        widget_start_row += 1

        rdo_primary_mapped_dark = ctk.CTkRadioButton(
            master=frm_widgets, text="Dark", variable=self.tk_primary_mapped_mode, value="Dark"
        )
        ...
        lbl_secondary_theme = ctk.CTkLabel(
            master=frm_widgets, text="Secondary theme", justify="right"
        )
        ...
        self.opm_secondary_theme = ctk.CTkOptionMenu(
            master=frm_widgets, variable=self.tk_secondary_theme, values=mod.user_themes_list()
        )
        self.opm_secondary_theme.grid(
            row=widget_start_row, column=1, padx=(0, 10), pady=(20, 5), sticky="w"
        )

        lbl_secondary_mode = ctk.CTkLabel(
            master=frm_widgets, text="Appearance mode", justify="right"
        )
        ...
        rdo_secondary_light = ctk.CTkRadioButton(
            master=frm_widgets, text="Light", variable=self.tk_secondary_mode, value="Light"
        )
        rdo_secondary_light.grid(row=widget_start_row, column=3, pady=(20, 5), sticky="w")
        widget_start_row += 1

        rdo_secondary_dark = ctk.CTkRadioButton(
            master=frm_widgets, text="Dark", variable=self.tk_secondary_mode, value="Dark"
        )
        ...
        widget_start_row += 1
        lbl_new_theme_name = ctk.CTkLabel(
            master=frm_widgets, text="New theme name", justify="right"
        )
        ...
        self.ent_new_theme_file = ctk.CTkEntry(
            master=frm_widgets, textvariable=self.tk_new_theme_file, width=160
        )
        self.ent_new_theme_file.grid(
            row=widget_start_row, column=1, padx=(0, 0), pady=(30, 20), sticky="w"
        )

        self.tk_open_on_merge = tk.IntVar(master=frm_widgets, value=0)
        self.swt_open_on_merge = ctk.CTkSwitch(
            master=frm_widgets,
            text="Open on merge",
            offvalue=0,
            onvalue=1,
        ...
        self.swt_open_on_merge.grid(
            row=widget_start_row, column=3, padx=(0, 0), pady=(30, 20), sticky="w"
        )

        if self.enable_tooltips:
            CTkToolTip(
                self.swt_open_on_merge,
                wraplength=400,
                justify="left",
                border_width=1,
                padding=(10, 10),
    ...
    def close_dialog(self, event=None):
        log.log_debug(
            log_text="Theme merger dialogue cancelled",
    ...
    def validate_and_merge(self):
        ...
        Merge button."""
        log.log_debug(
            log_text="Validate theme inputs and merge",
            class_name="ThemeMerger",
        ...
        if not primary_theme_name:
            ...
            log.log_debug(
                log_text="VALIDATION: You must select a Primary theme name",
                class_name="ThemeMerger",
                method_name="validate_and_merge",
            )
            return
        ...
        if not secondary_theme_name:
            ...
            log.log_debug(
                log_text="VALIDATION: You must select a Secondary theme name",
                class_name="ThemeMerger",
                method_name="validate_and_merge",
            )
            return
        ...
        if (
            primary_theme_name == secondary_theme_name
            and primary_appearance_mode == secondary_appearance_mode
        ):
            self.status_bar.set_status_text(
                "You cannot merge the same theme / appearance mode to itself."
            )
            log.log_debug(
                log_text="VALIDATION: You cannot merge the same theme / appearance mode to itself",
                class_name="ThemeMerger",
                method_name="validate_and_merge",
            )
            return
        ...
        if not mod.valid_theme_file_name(new_theme_file):
            ...
            log.log_debug(
                log_text="VALIDATION: Invalid characters in new theme file name!",
                class_name="ThemeMerger",
                method_name="validate_and_merge",
            )
            return
        ...
        if len(new_theme_file) == 0:
            self.status_bar.set_status_text(
                status_text="You must enter a theme name for the new theme."
            )
            return
        ...
        if new_theme_path.exists():
            self.status_bar.set_status_text(
                status_text=f"Theme file, {new_theme}, already exists - please choose another name!"
            )
            return
        self.status_bar.set_status_text(
            status_text=f"Creating theme, {new_theme_name}, from {primary_theme_name} "
            f" and {secondary_theme_name}."
        )
        mod.merge_themes(
            primary_theme_name=primary_theme_name,
            primary_mode=primary_appearance_mode,
            secondary_theme_name=secondary_theme_name,
            secondary_mode=secondary_appearance_mode,
